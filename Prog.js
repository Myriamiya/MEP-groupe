...
async function sendMessage(){
  ...
  attachedFiles.forEach(f=>{
    if(f.type.startsWith('image/')) userContent.push({type:'image',source:{type:'base64',media_type:f.type,data:f.base64}});
    else if(f.type==='application/pdf') userContent.push({type:'document',source:{type:'base64',media_type:'application/pdf',data:f.base64}});
  });
  ...
  const filesSnap=[...attachedFiles]; attachedFiles=[]; renderFilePreviews();
  ta.value=''; ta.style.height='auto'; updateSendBtn();
  const typingId='typing-'+Date.now(); appendTyping(typingId); isLoading=true; updateSendBtn();
  let sysPrompt="Tu es MEDIC IA, un assistant médical intelligent destiné aux professionnels de santé. Réponds en français de façon claire et professionnelle.";
  if(userSettings.verbose) sysPrompt+=" Développe tes explications de façon détaillée.";
  if(userSettings.sources) sysPrompt+=" Cite des références médicales reconnues quand c'est pertinent.";
  sysPrompt+=" Analyse les documents joints si présents.";
  try {
    const res=await fetch('https://api.anthropic.com/v1/messages',{
      method:'POST',headers:{'Content-Type':'application/json'},
      body:JSON.stringify({model:'claude-sonnet-4-6',max_tokens:1000,system:sysPrompt,messages:chatHistory})
    });
    ...
    if(!activeConvId){const title=(text||filesSnap[0]?.name||'Document').slice(0,48);activeConvId=addConversation(title,chatHistory)||null;}
    else{updateConversation(activeConvId,chatHistory);}
  } catch(err) {
    removeTyping(typingId);
    appendBubble('ai','⚠️ Une erreur s\'est produite. Veuillez réessayer.');
  }
